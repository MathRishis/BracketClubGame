...
export function sessionToken(req: Request) {
  return (
    req.headers
      ...
      ?.split(';')
      .map((part) => part.trim())
      .find((part) => part.startsWith('bc_session='))
      ?.slice(11) ?? null
  );
}
...
export async function accountFromRequest(
  ...
  const row = await database()
    .prepare(
      `SELECT a.id,a.username,a.display_name FROM sessions s JOIN accounts a ON a.id=s.account_id WHERE s.token_hash=? AND s.expires_at>?`,
    )
...
async function makeSession(accountId: string, secure: boolean) {
  ...
  await database()
    ...
    .bind(await digest(token), accountId, Date.now() + SESSION_AGE * 1000)
    .run();
...
export async function signup(req: Request, input: unknown) {
  ...
  await database()
    .prepare(
      'INSERT INTO accounts (id,username,display_name,password_hash,password_salt,created_at) VALUES (?,?,?,?,?,?)',
    )
    .bind(
      id,
      username,
  ...
  return {
    ...
    cookie: await makeSession(id, new URL(req.url).protocol === 'https:'),
  };
}

export async function login(req: Request, input: unknown) {
  const { username, password } = validateAccountInput({
    ...(input as object),
    displayName: usernameFrom(input),
  });
  const row = await database()
    .prepare(
      'SELECT id,username,display_name,password_hash,password_salt FROM accounts WHERE username=?',
    )
    .bind(username)
    .first<AccountRow>();
  if (
    !row ||
    (await passwordHash(password, row.password_salt)) !== row.password_hash
  )
  ...
  return {
    ...
    cookie: await makeSession(row.id, new URL(req.url).protocol === 'https:'),
  };
}
