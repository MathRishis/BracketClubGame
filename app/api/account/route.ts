...
export async function GET(req: Request) {
  try {
    return reply({ account: await accountFromRequest(req) });
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    if (input.action === 'logout')
      return reply({ account: null }, 200, await logout(req));
    let result;
    try {
      result =
        input.action === 'signup'
          ? await signup(req, input)
          : input.action === 'login'
            ? await login(req, input)
            : null;
    } catch (error) {
      throw new UserError(
