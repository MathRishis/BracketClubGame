...
export async function POST(req:Request){try{
 ...
 if(!Array.isArray(entries)||![4,8,16].includes(entries.length)||entries.some(x=>typeof x!=='string'||!x.trim()||x.trim().length>60))throw new UserError('Add 4, 8, or 16 contenders with names under 60 characters.');
